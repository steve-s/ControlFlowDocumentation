...
\end{figure}
    ...
    \section{Tables}        
        \note{CodeTables versus TypeTables}
        \note{CodeTables use RoutineContext annotation on AST elements}
        \note{Intra-procedural dependencies 
        handling \code{DependencyResolver} -- looks for routine context, 
        if it is not found invokes analysis. Checks for loops.}
    
    \section{Analyses}
        \subsection{Dead Code Elimination}
            \note{post order CFG traversal}
        
        \subsection{Aliasing Analysis}        
            \note{describe the simple rules. Justification: references are not used often.}
        
        \subsection{Constant Propagation}
            \note{Simple example of a DFA implementation}
