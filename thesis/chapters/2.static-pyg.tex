...
\chapter{Analysing PHP Code}
    ...
    the language has evolved and so have the ways how programmers were using it. 
...
\end{Verbatim}
    ...
    as well place any arbitrary code. Therefore a 
    the declaration can be wrapped in any control structure. 
    ...
    using \code{function\_exists} whether the functions where 
    ...
    popular system WordPress\cite{wordpress} still relies 
    on this pattern in parts of its code base.
    
    Although the mentioned pattern could be deemed as 
    reasonable and useful. This feature allows 
    to write very tricky code as in the following example 
...
\end{Verbatim}
    ...
    file explicitly. Newer versions of PHP allow to define 
    custom auto-loading routine that is invoked by the 
    runtime at any time an undefined class is referenced. 
    ...
    there is widely recognized format for documentation 
    ...
    comments, on the other hand, are of course not checked 
    by the runtime.
    
    The PHPDoc defines a fairly advanced syntax for expressing 
    type information. It allows to include multiple 
    primitive and class types, description of homogenous and heterogenous arrays as well 
